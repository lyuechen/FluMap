	...
	switch(num){
		case 0:{
			opt = {
		        ...
		        visualMap: {
		            type: 'piecewise',
		            pieces: [
		                {gt: 2000, color: 'darkred'},                        // (2000, Infinity]
		                {gt: 1300, lte: 2000, color: 'red', colorAlpha: 1},  // (1300, 2000]
		                ...
		                {lt: 50, color: 'red', colorAlpha: 0.2}          // (-Infinity, 50)
		            ]
		        },
		        ...
		        series: [
		            {
		                name: '国内各省确诊病例', // 系列名称，用于tooltip的显示，在 setOption 更新数据和配置项时用于指定对应的系列
		                zoom: 1.2, // 地图大小
		                type: 'map', // 类型地图
		                map: 'china', // 要使用的地图
		...
		case 2:{
			opt = {
			    ...
			    // 视觉映射方案：疫情颜色根据传染病疫情等级分类为5个级别：黄色-橙色-深橙色-红色-深红色
			    ...
			    visualMap: {
			        ...
			        realtime: true, // 拖拽时，是否实时更新地图
			        calculable: true, // 是否显示拖拽手柄，映射条可以拖拽调整要映射的范围
			        inRange: {
			    ...
			    series: [
			        {
				        ...
				        label: { // 图形上的文本标签，地图默认显示数据名
				            ...
				            //formatter: '{b}', // b是数据名，c是数据值
				            formatter: function (params) {
				            //公布了数据 且 数据累计数据大于1万的显示国家名称
					            if(params.data && params.data.value > 10000 || params.name == '中国') 
					                return params.name;
					            else
					                return '';
				            }
